...
function configureOutbound(handle, stream, body, depth = 0) {
  ...
  if (typeof body === 'string') {
    handle.attachSource(Buffer.from(body, 'utf8'));
    return;
  }
...
class QuicStream {
  ...
  get writer() {
    ...
    async function write(chunk, options) {
      ...
      if (closed) throw new ERR_INVALID_STATE('Writer is closed');
      chunk = toUint8Array(chunk);
      const result = handle.write([chunk]);
      if (result === undefined) {
    ...
    function writevSync(chunks) {
      ...
      if (result === undefined) return false;
      for (const c of chunks) totalBytesWritten += c.byteLength;
      return true;
    }
    ...
    const writer = {
      ...
      get desiredSize() {
        ...
        return Number(stream.#state.writeDesiredSize);
      },
      writeSync,
      write,
      writevSync,
      ...
      [drainableProtocol]() {
        if (closed || errored) return null;
        if (Number(stream.#state.writeDesiredSize) > 0) return null;
        drainWakeup = PromiseWithResolvers();
