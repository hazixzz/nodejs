...
// This is the exact bug Qard (Stephen Belanger) identified: CPED is a shared
// AsyncContextFrame Map. Its identity (address) changes when ANY ALS store
// changes, not just the heap profile labels store. The old address-based lookup
// would fail because the CPED address at allocation time differs from the one
// registered with withHeapProfileLabels. The new CPED-storage approach stores
// the full CPED value on each sample at allocation time and resolves labels
// at profile-read time via Map lookup — immune to address changes.
{
